...
class Employee extends CI_Controller
{
	...
	{
		if ($this->session->userdata('user_login_access') != 1) redirect(base_url() . 'login', 'refresh');
		if ($this->session->userdata('user_login_access') == 1) $data = array();
...
//This function is used to add a new employee.
...
//This function is used to save a new employee.
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($this->employee_model->Does_email_exists($email) && $password != $confirm) {
					...
					if ($_FILES['image_url']['name']) {
						...
						$config = array('file_name' => $new_file_name, 'upload_path' => "./assets/images/users", 'allowed_types' => "gif|jpg|png|jpeg", 'overwrite' => False, 'max_size' => "20240000", // Can be set to particular file size , here it is 2 MB(2048 Kb)
							'max_height' => "800", 'max_width' => "800");
						...
						if (!$this->upload->do_upload('image_url')) {
							...
							$data = array();
							$data = array('em_id' => $emrand, 'em_code' => $eid, 'des_id' => $deg, 'dep_id' => $dept, 'first_name' => $fname, 'last_name' => $lname, 'em_email' => $email, 'em_password' => $password, 'em_role' => $role, 'em_gender' => $gender, 'status' => 'ACTIVE', 'em_phone' => $contact, 'em_birthday' => $dob, 'em_joining_date' => $joindate, 'em_contact_end' => $leavedate, 'em_image' => $img_url, 'em_nid' => $nid, 'em_blood_group' => $blood);
							if ($id) {
						...
						$data = array();
						$data = array('em_id' => $emrand, 'em_code' => $eid, 'des_id' => $deg, 'dep_id' => $dept, 'first_name' => $fname, 'last_name' => $lname, 'em_email' => $email, 'em_password' => $password, 'em_role' => $role, 'em_gender' => $gender, 'status' => 'ACTIVE', 'em_phone' => $contact, 'em_birthday' => $dob, 'em_joining_date' => $joindate, 'em_contact_end' => $leavedate, 'em_address' => $address, 'em_nid' => $nid, 'em_blood_group' => $blood);
						if ($id) {
							$success = $this->employee_model->Update($data, $id);
							#$this->session->set_flashdata('feedback','Successfully Updated');
							echo "Successfully Updated";
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($_FILES['image_url']['name']) {
					...
					if (!$this->upload->do_upload('image_url')) {
						...
						$img_url = $path['file_name'];
						$data = array();
						$data = array('em_code' => $eid, 'des_id' => $deg, 'dep_id' => $dept, 'first_name' => $fname, 'last_name' => $lname, 'em_email' => $email, 'em_role' => $role, 'em_gender' => $gender, 'status' => $status, 'em_phone' => $contact, 'em_birthday' => $dob, 'em_joining_date' => $joindate, 'em_contact_end' => $leavedate, 'em_image' => $img_url, 'em_address' => $address, 'em_nid' => $nid, 'em_blood_group' => $blood);
						if ($id) {
							$success = $this->employee_model->Update($data, $id);
							$this->session->set_flashdata('feedback', 'Successfully Updated');
							echo "Successfully Updated";
					...
					$data = array();
					$data = array('em_code' => $eid, 'des_id' => $deg, 'dep_id' => $dept, 'first_name' => $fname, 'last_name' => $lname, 'em_email' => $email, 'em_role' => $role, 'em_gender' => $gender, 'status' => $status, 'em_phone' => $contact, 'em_birthday' => $dob, 'em_joining_date' => $joindate, 'em_contact_end' => $leavedate, 'em_address' => $address, 'em_nid' => $nid, 'em_blood_group' => $blood);
					if ($id) {
						$success = $this->employee_model->Update($data, $id);
						$this->session->set_flashdata('feedback', 'Successfully Updated');
						echo "Successfully Updated";
...
//This function records the permanent address of the user.
	public function Parmanent_Address()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('emp_id' => $em_id, 'city' => $parcity, 'country' => $parcountry, 'address' => $paraddress, 'type' => 'Permanent');
				if (!empty($id)) {
...
//This function is used to record the present address of the user.
	public function Present_Address()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('emp_id' => $em_id, 'city' => $prescity, 'country' => $prescountry, 'address' => $presaddress, 'type' => 'Present');
				if (empty($id)) {
					$success = $this->employee_model->AddParmanent_Address($data);
					$this->session->set_flashdata('feedback', 'Successfully Added');
					echo "Successfully Updated";
...
//This function is used to add education level of an employee.
	public function Add_Education()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('emp_id' => $em_id, 'edu_type' => $certificate, 'institute' => $institute, 'result' => $eduresult, 'year' => $eduyear);
				if (empty($id)) {
...
//This function is used to save social status of an employee.
	public function Save_Social()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('emp_id' => $em_id, 'facebook' => $facebook, 'twitter' => $twitter, 'google_plus' => $google, 'skype_id' => $skype);
				if (empty($id)) {
...
//This function is used to add an employee experience.
	public function Add_Experience()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('emp_id' => $em_id, 'exp_company' => $company, 'exp_com_position' => $position, 'exp_com_address' => $address, 'exp_workduration' => $start);
				if (empty($id)) {
...
//This function is used to define an employee disciplinary level.
...
//This function adds employee disciplinary data.
	public function add_Desciplinary()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('em_id' => $em_id, 'action' => $warning, 'title' => $title, 'description' => $details);
				if (empty($id)) {
...
//Add customers bank information.
	public function Add_bank_info()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('em_id' => $em_id, 'holder_name' => $holder, 'bank_name' => $bank, 'branch_name' => $branch, 'account_number' => $number, 'account_type' => $account);
				if (empty($id)) {
...
//Add reset password for hr.
	public function Reset_Password_Hr()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($onep == $twop) {
				$data = array();
				$data = array('em_password' => sha1($onep));
...
//Function to add a dew department
	...
	public function Assign_leave()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('emp_id' => $emid, 'type_id' => $type, 'day' => $day, 'total_day' => '0', 'year' => $year);
		...
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($_FILES['file_url']['name']) {
					...
					$config = array('file_name' => $new_file_name, 'upload_path' => "./assets/images/users", 'allowed_types' => "gif|jpg|png|jpeg|pdf|doc|docx|xml|text|txt", 'overwrite' => False, 'max_size' => "40480000");
					...
					if (!$this->upload->do_upload('file_url')) {
						...
						$img_url = $path['file_name'];
						$data = array();
						$data = array('em_id' => $em_id, 'file_title' => $filetitle, 'file_url' => $img_url);
...
//Delete an employee disciplinary.
...
//THis function is used to add an employee salary.
	public function Add_Salary()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('emp_id' => $em_id, 'type_id' => $type, 'total' => $total);
				if (!empty($sid)) {
					...
					if (!empty($aid)) {
						$data1 = array();
						$data1 = array('salary_id' => $sid, 'basic' => $basic, 'medical' => $medical, 'house_rent' => $houserent, 'conveyance' => $conveyance);
						$success = $this->employee_model->Update_Addition($aid, $data1);
					}
					if (!empty($did)) {
						$data2 = array();
						$data2 = array('salary_id' => $sid, 'provident_fund' => $provident, 'bima' => $bima, 'tax' => $tax, 'others' => $others);
					...
					#echo "Successfully Added";
					$data1 = array();
					$data1 = array('salary_id' => $insertId, 'basic' => $basic, 'medical' => $medical, 'house_rent' => $houserent, 'conveyance' => $conveyance);
					$success = $this->employee_model->Add_Addition($data1);
					$data2 = array();
					$data2 = array('salary_id' => $insertId, 'provident_fund' => $provident, 'bima' => $bima, 'tax' => $tax, 'others' => $others);
...
//This function is used to send an email using an SMTp server.
	public function confirm_mail_send($email, $pass_hash)
	{
		$config = array('protocol' => 'smtp', 'smtp_host' => 'ssl://smtp.googlemail.com', 'smtp_port' => 465, 'smtp_user' => 'mail.imojenpay.com', 'smtp_pass' => '');
