...
class Payroll extends CI_Controller
{
	...
	public function Add_Sallary_Type()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('salary_type' => $type, 'create_date' => $createdate);
				if (empty($id)) {
	...
	//The function that will generate salary.
	...
	public function Add_Sallary_Pay()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('emp_id' => $emid, 'month' => $month, 'paid_date' => $paydate, 'total_days' => $totalday, 'basic' => $basic, 'loan' => $loan, 'total_pay' => $total, 'status' => $status, 'paid_type' => $paid_type);
				if (empty($id)) {
					...
					if (empty($loanid)) {
						...
						if (!empty($loanvalue)) {
							...
							$data = array();
							$data = array('emp_id' => $emid, 'loan_id' => $loanid, 'loan_number' => $number, 'install_amount' => $loan, /*'pay_amount' => $payment,*/
								'app_date' => $paydate, /*'receiver' => $receiver,*/
								'install_no' => $period/*'notes' => $notes*/);
							...
							$data = array();
							$data = array('total_pay' => $totalpay, 'total_due' => $totaldue, 'install_period' => $period, 'status' => 'Done');
	...
	public function Add_Salary()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('emp_id' => $em_id, /*'type_id' => $type,*/
					'basic' => $basic);
				if (!empty($sid)) {
					...
					#echo "Successfully Added";
					$data1 = array();
					$data1 = array('salary_id' => $sid, 'medical' => $medical, 'house_rent' => $houserent, 'bonus' => $bonus);
					...
					#echo "Successfully Added";
					$data1 = array();
					$data1 = array('salary_id' => $insertId, 'medical' => $medical, 'house_rent' => $houserent, 'bonus' => $bonus);
	...
	public function load_employee_Invoice_by_EmId_for_pay()
	{
        ...
        <div class='col-md-12'>
			...
			echo "</td>
                                        ...
                                        <tr>
                                            <td>Days Worked</td>
                                            <td>:" . ceil($salary_info->total_days / 8);
			echo "</td>";
	...
	public function pay_salary_add_record()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array();
				$data = array('emp_id' => $emid, 'month' => $month, 'year' => $year, 'paid_date' => $paydate, 'total_days' => $hours_worked, 'basic' => $basic, 'loan' => $loan, 'total_pay' => $total_paid, 'addition' => $addition, 'diduction' => $diduction, 'status' => $status, 'paid_type' => $paid_type,);
					...
					if ($payment_status == "Paid") {
						...
						if ($success && $status == "Paid") {
							...
							if (!empty($loan_info)) {
								...
								$data = array();
								$data = array('emp_id' => $emid, 'loan_id' => $loan_id, 'loan_number' => $number, 'install_amount' => $loan, /*'pay_amount' => $payment,*/
									'app_date' => $paydate, /*'receiver' => $receiver,*/
									'install_no' => $period/*'notes' => $notes*/);
								...
								$data = array();
								$data = array('total_pay' => $totalpay, 'total_due' => $totaldue, 'install_period' => $period, 'status' => $loan_status);
